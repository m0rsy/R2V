...
class _SignInState extends State<SignIn> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      if (e.message == 'EMAIL_NOT_VERIFIED') {
        await _promptVerifyEmail(email);
        return;
      }
  ...
  /// Shown when login is rejected with EMAIL_NOT_VERIFIED. Lets the user resend
  /// a fresh code and jump to the verification screen.
  Future<void> _promptVerifyEmail(String email) async {
    ...
    messenger.showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
          ...
          onPressed: () async {
            try {
              final res = await r2vEmailVerification.requestCode(email);
              if (!mounted) return;
